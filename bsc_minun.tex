...
Kaksistoista vuotta kirjan julkaisun jälkeen pidetyssä haastattelussa kirjailijat sanovat, että useimmat avoimen lähdekoodin liikepalveluväylät \textit{(enterprise service bus, ESB)} ovat omaksuneet teoksessa esitellyn mallikielen \texit{(pattern language)} \citep{Zimmermann2016}.
...
\begin{enumerate}
   \item Sanomajärjestelmät \texit{(messaging systems)}
   \item Sanomakanavat \texit{(messaging channels)}
   \item Sanomien rakentaminen \texit{(message construction)}
   \item Sanomien reititys \texit{(message routing)}
   \item Sanomien muokkaus \texit{(message transformation)}
   \item Sanomien päätepisteet \texit{(messaging endpoints)}
   \item Järjestelmän hallinta \texit{(systems management)}
\end{enumerate}
...
\begin{itemize}
   ...
   \item Putket ja suodattimet \textit{(Pipes and filters)}
      Putket ja suodattimet suunnittelumallissa putkena toimivat edellä mainitut kanavat ja suodattimet ovat pieniä itsenäisiä prosessointiaskeleita joilla ei ole kytköksiäs muihin prosessointiaskeleihin. Kanavat yhdistävät nämä suodattime toisiinssa ja suodattimet voivat toimia yksin tai olla yhdistettynä toisiinsa. Vaihtoehtoisesti putkena voi toimia muistissa oleva jono. Suodattimien rajapinta pitää olla tarpeeksi yksinkertainen, että putkien implementaation on vaihdettavissa ja kiinteitä kytköksiä muihin suodattimiin tai putkiin ei synny.
Asynkroninen viestintä komponenttien välillä mahdollistaa suodattimien samanaikaisen käytön.
      ...
      Suodattamien käyttö ei ole täysin ilmaista. Suodattamien skaalautuvuuden haasteena on useiden putkien käyttö; samoakanavat eivät ole täysin ilmaisia kuten aikasemmassa suunnittelumallissa on mainittu. Myös sanomien julkaisu ei ole ilmaista, koska viesti pitää käsitellä kanavan formaatista sovellukseen käyttämään formaattiin ja sama prosessi pitää tehdä takaperin sanoman poistuessa.
\end{itemize}

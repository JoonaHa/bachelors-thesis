...
\begin{enumerate}
   \item Järjestelmät \texit{(messaging systems)}
   \item Kanavat \texit{(messaging channels)}
   \item Rakentaminen \texit{(message construction)}
   \item Reititys \texit{(message routing)}
   \item Muokkaus \texit{(message transformation)}
   \item Päätepisteet \texit{(messaging endpoints)}
   \item Järjestelmän hallinta \texit{(systems management)}
\end{enumerate}

\section{Sanomajärjestelmät}
Järjestelmät kategoria toimii kattokattegoriana muille kategoroille ja tarjoaa konseptit ja termit muille suunnittelualleille. 
Kategoria esittelee lähtökohtaiset pohja suunnittelumallit minkä päälle muut luvut rakentavat.

\begin{itemize}
   \item Message Channel
\end{itemize}

Neljästäkymmenestäkahdeksasta modernista integraatioalusta kymmenen hyödyntää putket-ja-suodattimet arkkitehtuurityyliä \citep{Freire2019}.
